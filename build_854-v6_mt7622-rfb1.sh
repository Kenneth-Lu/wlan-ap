...
set -e
#***************************************************************************
#Name : build_854-v6_mt7622-rfb1.sh
#Description : This is the master script to build openwrt image for 854-v6
#with opensync
#***************************************************************************
ROOT_PATH=${PWD}
...
{
   echo ""
   echo "build_854-v6_mt7622-rfb1.sh : This is the master script to build openwrt image for mt7622-rfb1 with opensync"
   echo "Syntax: ./build_854-v6_mt7622-rfb1.sh [build|rebuild|help]"
   echo ""
   echo "Prerequisite:"
   ...
   echo "rebuild  re-compile and build 854-v6 image"
   echo "help     help list"
   echo ""
   exit 1
}
...
genConfig()
{
   ...
   cd $BUILD_DIR
   python$PY_VERSION $BUILD_DIR/scripts/gen_config.py mt7622-rfb1 wlan-ap-consumer wifi-854-v6 || exit 1
   cd ..
}
...
applyPatches()
{
    ...
    cd $ROOT_PATH
    if [ ! -d "../device-vendor-adtran/additional-patches/target/854-v6/patches/openwrt/" ]; then
       echo "device-vendor-adtan directory doesn't have openwrt patches. exit!!"
#       exit 1
    fi

    if [ ! -d "../device-vendor-adtran/additional-patches/target/854-v6/patches/opensync/" ]; then
       echo "device-vendor-adtan directory doesn't have opensync patches. exit!!"
       exit 1
    fi

    cp ../device-vendor-adtran/additional-patches/target/854-v6/patches/opensync/* $ROOT_PATH/feeds/wlan-ap-consumer/opensync/patches/.
#    cp -r ../device-vendor-adtran/additional-patches/target/854-v6/patches/openwrt/* $ROOT_PATH/feeds/wlan-ap-consumer/additional-patches/patches/openwrt/.
...
applyOpenwrtPatches()
{
    TARGET_SPECFIC_OWRT_PATCH_PATH="../../device-vendor-adtran/additional-patches/target/854-v6/patches/owrtPatches"
    cd $BUILD_DIR
...
addPkgFeeds()
{
    TARGET_SPECFIC_FEEDS_PATH="../../device-vendor-adtran/additional-patches/target/854-v6/feeds"
